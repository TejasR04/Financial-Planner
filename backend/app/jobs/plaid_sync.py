...
from app.services.plaid_sync_service import (
    release_sync_lease, sync_all_linked_institutions, try_acquire_sync_lease,
)
...
async def run_once() -> int:
    ...
    try:
        if not settings.plaid_client_id or not settings.plaid_secret:
            raise RuntimeError("Plaid credentials are required for scheduled sync")
        _fernet()
        async with engine.connect() as raw_connection:
            ...
            if not await try_acquire_sync_lease(connection):
                ...
                return 0
            try:
                failures = await sync_all_linked_institutions()
