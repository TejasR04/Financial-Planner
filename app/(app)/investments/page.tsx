...
export default function InvestmentsPage() {
  ...
  return (
    <PageContainer>
      ...
      <p className="mt-3 text-xs text-muted-foreground">Account balances and reported holdings are two views of the same portfolio; do not add them together. Differences can reflect unreported positions, cash, or different update times. Balance changes include deposits and withdrawals and are not investment returns.{dashboard && Number(dashboard.excluded_gain_loss_value ?? 0) > 0 ? ` ${formatCurrency(Number(dashboard.excluded_gain_loss_value))} in cash or positions without cost basis is excluded from gain/loss.` : ""}</p>
      ...
      <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-3">
        <Panel className="xl:col-span-2">
          <PanelHeader title="Positions" description="Reported positions. A dash means cost basis is unavailable or gain/loss is excluded for cash." />
          {dashboard?.holdings.length ? <div className="overflow-x-auto"><table className="w-full border-collapse text-[13px]">
            ...
            <tbody>{dashboard.holdings.map((holding) => <tr key={`${holding.account_id}-${holding.symbol}`} className="border-b border-border/60 last:border-0">
              <td className="px-4 py-3"><p className="font-medium text-foreground">{holding.symbol}</p><p className="text-[11px] capitalize text-muted-foreground">{ASSET_CLASS_LABEL[holding.asset_class] ?? holding.asset_class}</p></td>
