...
import { formatTimestamp } from "@/lib/dashboard-data-helpers";
  ...
  return (
    <PageContainer>
      ...
      <Panel className="mt-4">
        ...
        {institutions.length === 0 ? <p className="px-4 py-8 text-center text-sm text-muted-foreground">No institutions linked yet. Link a U.S. bank or add an account manually.</p> : <ul className="divide-y divide-border">{institutions.map((institution) => <li key={institution.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-3"><div><p className="text-sm font-medium text-foreground">{institution.name}</p><p className="mt-0.5 text-xs text-muted-foreground">{institution.accountCount} account{institution.accountCount === 1 ? "" : "s"} · {institution.lastSyncedAt ? `Synced ${formatTimestamp(institution.lastSyncedAt)}` : "Not synced yet"}</p></div><div className="flex items-center gap-2"><span className={institution.status === "healthy" ? "text-xs font-medium text-positive" : "text-xs font-medium text-warning"}>{institutionStatusLabel(institution)}</span><Button variant="outline" size="xs" onClick={() => void syncInstitution(institution)} disabled={pendingActionId !== null}><RefreshCw className={pendingActionId === institution.id ? "animate-spin" : undefined} />Sync</Button>{institution.status !== "healthy" && <PlaidLinkButton label="Reconnect" institutionId={institution.id} size="xs" variant="outline" />}<Button variant="ghost" size="xs" onClick={() => void unlinkInstitution(institution)} disabled={pendingActionId !== null}><Unlink />Unlink</Button></div></li>)}</ul>}
      </Panel>
