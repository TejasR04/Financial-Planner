...
export function SpendingPaceChart({ summary, month, budget, loading }: { summary: ApiBudgetSummary | null; month: string; budget: number; loading: boolean }) {
  ...
  const averageLabel = `Average (${summary?.average_month_count ?? 0} completed months)`;
  const current = summary?.daily_spending ?? [];
  const previous = summary?.average_daily_spending ?? [];
  ...
  return <Panel>
    <PanelHeader title="Spending through the month" description={`${label} compared with your historical average · includes pending spending and reimbursements`} />
    <div className="px-4 pt-3 text-sm">
      {loading ? "Loading spending history…" : difference !== null ? <><strong className="tabular-nums">{formatCurrency(Math.abs(difference))} {difference > 0 ? "more" : difference < 0 ? "less" : "difference"}</strong><span className="text-muted-foreground"> than your average through day {last.day}</span></> : <span className="text-muted-foreground">A completed month of history is needed for comparison.</span>}
    </div>
    <div className="h-64 px-2 pt-3">
      {!loading && anyData ? <ResponsiveContainer width="100%" height="100%"><LineChart data={points} margin={{ top: 20, right: 24, left: 5, bottom: 5 }}>
        ...
        <Line type="linear" dataKey="previous" name={averageLabel} stroke="var(--muted-foreground)" strokeDasharray="5 5" dot={false} connectNulls={false} isAnimationActive={false} />
    ...
    <div className="flex gap-4 px-4 py-2 text-xs text-muted-foreground"><span>{label} — solid</span><span>{averageLabel} — dashed</span></div>
    <p className="px-4 pb-3 text-xs text-muted-foreground">Data as of {summary?.as_of ?? "today"}. Average uses up to 12 completed months before the selected month, excluding the current and first partial month{summary?.average_period_start && summary.average_period_end ? ` (${summary.average_period_start.slice(0, 7)} to ${summary.average_period_end.slice(0, 7)})` : ""}. Shorter months carry their closing total through day 31. Imported history may be incomplete; budget limits use current settings.</p>
  </Panel>;
}
