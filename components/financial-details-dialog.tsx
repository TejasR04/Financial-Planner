...
export function FinancialDetailsDialog({ account, onClose }: { account: Account | null; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    if (debt) {
      api.accounts.liability(account.id).then((row) => setValues(row ? {
        ...
        interest_rate: String(Number(row.interest_rate) * 100),
        term_months: String(row.term_months),
    ...
    try {
      if (debt) {
        await api.accounts.saveLiability(account!.id, {
          ...
          interest_rate: String(Number(values.interest_rate) / 100),
  ...
  return (
    <DialogShell onClose={onClose} ariaLabelledBy="financial-details-title" panelClassName="max-w-lg rounded-lg bg-card p-4">
      ...
      <p className="mt-1 text-xs text-muted-foreground">{debt ? "Terms supplement the account balance; they never change net worth." : "Positions explain allocation; their values are not added again to the account balance."}</p>
