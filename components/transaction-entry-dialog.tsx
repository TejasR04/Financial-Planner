...
export function TransactionEntryDialog({ open, accounts, onClose, onSaved }: { open: boolean; accounts: Account[]; onClose: () => void; onSaved: (message: string) => void }) {
  ...
  if (!open) return null;
  const confirmedDuplicateCount = previewRows.filter((row) => row.likely_duplicate && row.force_import).length;
  ...
  const submitManual = async (event: React.FormEvent) => {
    ...
    try {
      await api.transactions.create({
        ...
        amount: String(direction === "out" ? -Math.abs(parsedAmount) : Math.abs(parsedAmount)),
        type,
        status: "cleared",
      });
      setMerchant("");
  ...
  const overrides = (): ApiCsvImportOverride[] => previewRows.map((row) => ({
    ...
    include: !row.likely_duplicate || Boolean(row.force_import),
    force_import: Boolean(row.force_import),
  ...
  return <DialogShell onClose={onClose} closeDisabled={saving} ariaLabelledBy="transaction-entry-title" panelClassName="max-w-4xl">
    ...
    {mode === "manual" ? <form onSubmit={submitManual} className="space-y-3 p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3"><label className="text-xs font-medium text-muted-foreground">Category<input required value={category} onChange={(event) => setCategory(event.target.value)} className={inputClass} placeholder="groceries" /></label><label className="text-xs font-medium text-muted-foreground">Type<select value={type} onChange={(event) => { const next = event.target.value as ApiTransaction["type"]; setType(next); setDirection(next === "income" ? "in" : "out"); }} className={inputClass}><option value="expense">Expense / refund</option><option value="income">Income</option><option value="transfer">Transfer</option><option value="credit_card_payment">Credit card payment</option><option value="contribution">Contribution</option></select></label><label className="text-xs font-medium text-muted-foreground">Direction<select value={direction} onChange={(event) => setDirection(event.target.value as "out" | "in")} className={inputClass}><option value="out">Money out</option><option value="in">Money in</option></select></label><label className="text-xs font-medium text-muted-foreground">Amount<input required type="number" step="0.01" min="0.01" inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} className={inputClass} placeholder="0.00" /></label></div>
      ...
      {error && <p role="alert" className="text-xs text-destructive">{error}</p>}
      {preview && previewRows.some((row) => row.likely_duplicate) && <div className="rounded-md border border-warning/30 bg-warning/5 p-3 text-xs"><p className="font-medium">Review suspected duplicates</p><p className="mt-1 text-muted-foreground">Select only purchases you recognize as separate real transactions. Confirming a row imports it as another occurrence, even when its details match an earlier import.</p><div className="mt-2 space-y-1">{previewRows.filter((row) => row.likely_duplicate).map((row) => <label key={row.row_number} className="flex items-center gap-2"><input type="checkbox" checked={Boolean(row.force_import)} onChange={(event) => setPreviewRows((rows) => rows.map((item) => item.row_number === row.row_number ? { ...item, force_import: event.target.checked } : item))} /><span>Import row {row.row_number}: {row.merchant} ({row.amount})</span></label>)}</div></div>}
      <div className="flex justify-end gap-2"><Button type="button" variant="outline" size="sm" onClick={onClose} disabled={saving}>Cancel</Button><Button type="submit" size="sm" disabled={saving || !accounts.length || (!!preview && !previewDirty && preview.importable_count + confirmedDuplicateCount === 0)}>{saving ? "Checking…" : !preview ? "Review import" : previewDirty ? "Recheck changes" : `Import ${preview.importable_count + confirmedDuplicateCount}`}</Button></div>
    </form>}
  </DialogShell>;
}
