...
export function TransactionEntryDialog({ open, accounts, onClose, onSaved }: { open: boolean; accounts: Account[]; onClose: () => void; onSaved: (message: string) => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/20 p-4 backdrop-blur-sm" onMouseDown={() => !saving && onClose()}>
    <div role="dialog" aria-modal="true" aria-labelledby="transaction-entry-title" className="w-full max-w-lg rounded-xl border border-border bg-popover shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
